...
app.post('/auth/register', async (req, res) => {
    try {
        ...
        if (_result.rows.length > 0) res.status(400).json({ error: 'error' });
...
app.post('/auth/login', async (req, res) => {
    try {
        ...
        if (result.rows.length === 0) res.status(400).json({ error: 'error' });

        const dbPass = result.rows[0].password_hash;

        const checkPass = await bcrypt.compare(password, dbPass);

        if (!checkPass) res.status(400).json({ error: 'error' });
        ...
        const secret = JWT_SECRET as string; // например: "my_super_secret_key"
